import { spawnBlood, explodeGeneric } from './physics-core.js';
import { updateHUD, doGameOver } from './ui.js';
import { soundManager } from './sound-manager.js';

export function updateZombies(state, timestamp, scale) {
    ...
    for (let i = zombies.length - 1; i >= 0; i--) {
        ...
        if (dToP < 30) {
            ...
            z.hp -= 0.5 * scale; spawnBlood(state, player.x, player.y, 1);
            if (z.hp <= 0) zombies.splice(i, 1);
            if (player.hp <= 0) doGameOver(state);
            updateHUD(state);
        }
        ...
        if (z.type === 'shooter' || z.type === 'shotgunner' || z.type === 'ar_gunner' || z.type === 'flamethrower') {
            ...
            handleShooterAI(state, z, angle);
        }
        if (z.type === 'exploder' && dToP < 50) { zombies.splice(i, 1); explodeGeneric(state, z.x, z.y, 30, 200, true); }
    }
}

export function handleShooterAI(state, z, angle) {
    ...
    if (z.type === 'shooter' && (!z.lastShot || now - z.lastShot > 2000)) {
        ...
        soundManager.playFile('pistol', 0.2); z.lastShot = now;
    }
    if (z.type === 'shotgunner' && (!z.lastShot || now - z.lastShot > 3000)) {
        ...
        soundManager.playFile('shotgun', 0.3); z.lastShot = now;
    }
    if (z.type === 'ar_gunner') {
        if (z.isReloading) { if (now - z.reloadStart > 5000) { z.ammo = 75; z.isReloading = false; } }
            ...
            z.ammo--; soundManager.playFile('rifle', 0.1); z.lastShot = now;
        }
    }
    if (z.type === 'flamethrower' && dToP < 120) {
        if (!z.lastShot || now - z.lastShot > 50) {
            ...
            if (dToP < 110) { player.hp -= 0.3; updateHUD(state); }
            z.lastShot = now;
        }
    }
}
